...
# Input
file_path = meaning_folder / "id_to_meaning_list_tf_idf.json"
mask_name = "doge"
mask_extension = "png"
mask_file_path = mask_folder / f"{mask_name}.{mask_extension}"

# Output
output_file_path = results_folder / "meaning_wordcloud.png"
output_file_color_path = results_folder / "meaning_wordcloud_color.png"
...
# mask_bw = np.where(mask_bw == 0, 255, 0)
mask_bw = np.where(mask_bw > 230, 255, 0)

# Save mask
cv2.imwrite(mask_folder / f"{mask_name}_bw.png", mask_bw)

with open(file_path, "r", encoding="utf-8") as file:
    id_to_meaning_list_tf_idf = json.load(file)

# Join all the tokens
values = list(id_to_meaning_list_tf_idf.values())
